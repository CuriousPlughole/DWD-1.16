...
public class QuickSandPoolFeature extends Feature<ProbabilityConfig> {

	public QuickSandPoolFeature(Codec<ProbabilityConfig> codec) {
		super(codec);
	}
	...
	public boolean generate(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos,
		...
		BlockState QSBlockState = BlockInit.SKARO_QUICKSAND.get().getDefaultState();

		pos = reader.getHeight(Type.WORLD_SURFACE_WG, pos);
             ...
             for(int l2 = 0; l2 < 16; ++l2) {
                for(int k = 0; k < 8; ++k) {
                   ...
                   if (flag) {
                      ...
                      if (k < 4 && !material.isSolid() && reader.getBlockState(pos.add(k1, k, l2)) != QSBlockState) {
                         return false;
                      }
                   }
                }
             ...
             for(int i3 = 0; i3 < 16; ++i3) {
                for(int i4 = 0; i4 < 8; ++i4) {
                   if (aboolean[(l1 * 16 + i3) * 8 + i4]) {
                      reader.setBlockState(pos.add(l1, i4, i3), i4 >= 4 ? Blocks.CAVE_AIR.getDefaultState() : QSBlockState, 2);
                   }
                }
             }
          }
                ...
                for(int k3 = 0; k3 < 16; ++k3) {
                   for(int k4 = 0; k4 < 8; ++k4) {
                      ...
                      if (flag1 && (k4 < 4 || rand.nextInt(2) != 0) && reader.getBlockState(pos.add(j2, k4, k3)).getMaterial().isSolid()) {
                    	  BlockState boundary = rand.nextBoolean() ? QSBlockState : BlockInit.SKARO_DIRT.get().getDefaultState();
                         reader.setBlockState(pos.add(j2, k4, k3), boundary, 2);
