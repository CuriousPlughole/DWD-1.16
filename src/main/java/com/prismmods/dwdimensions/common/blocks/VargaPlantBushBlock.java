...
public class VargaPlantBushBlock extends BushBlock implements IGrowable {
	...
	public VargaPlantBushBlock(Block.Properties properties) {
		super(properties);
		this.setDefaultState(this.stateContainer.getBaseState().with(AGE, Integer.valueOf(0)));
	}

	public ItemStack getItem(IBlockReader worldIn, BlockPos pos, BlockState state) {
		return new ItemStack(ItemInit.VARGA_BERRIES.get());
	}
		...
		if (!worldIn.isRemote && worldIn.getDifficulty() != Difficulty.PEACEFUL) {
			if (entityIn instanceof LivingEntity) {
				...
				if (!livingentity.isInvulnerableTo(DamageSource.MAGIC)) {
					livingentity.addPotionEffect(new EffectInstance(Effects.POISON, 400));
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player,
		...
		if (!flag && player.getHeldItem(handIn).getItem() == Items.BONE_MEAL) {
			...
			spawnAsEntity(worldIn, pos, new ItemStack(ItemInit.VARGA_BERRIES.get(), j + (flag ? 1 : 0)));
	...
	protected boolean isValidGround(BlockState state, IBlockReader worldIn, BlockPos pos) {
		...
		return block == Blocks.DIRT || block == Blocks.COARSE_DIRT || block == Blocks.SAND || block == Blocks.RED_SAND
				|| DWDimensions.sand_valid_ground.contains(block) || DWDimensions.dirt_valid_ground.contains(block);
	}
}
